...
function read_ts (filename) {

  let program = ts.createProgram ([filename], {});
  // let checker = program.getTypeChecker();
  let sourceFile = program.getSourceFile(filename);

  sourceFile.forEachChild (sourceFile, node => {
    let name = "";
    if (ts.isFunctionDeclaration (node)) {
      console.log ("function declaration ", node.name.text);
...
async function mymain (filenames) {
  ...
  try {
    ...
    const myobj = {"about": {"__proto__": {"role": "admin"}}};
    let s = '{"about": {"__proto__": {"role": "admin"}}}';
    // s = '{"about": "mayval"}';
    console.log (s);
